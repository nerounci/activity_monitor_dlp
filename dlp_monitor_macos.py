...
Запуск:
    pip install psutil watchdog flask pyobjc-framework-Quartz pyobjc-framework-AppKit
    python dlp_monitor_macos.py [--port 5000] [--db dlp_monitor.db] [--no-browser]

reqs: macOS 12+, Python 3.10+
Дашборд:   http://localhost:5000

Ключевые отличия от Windows-версии:
  - UserActivityMonitor  → NSWorkspace + CGWindowListCopyWindowInfo (AppKit/Quartz)
  - USBMonitor           → IOServiceMatching + IOIterator через pyobjc (IOKit)
  - FileSystemMonitor    → watchdog автоматически переключается на FSEvents backend
  - SKIP_PREFIXES        → macOS-пути вместо C:\\Windows, C:\\Program Files
  - SUSPICIOUS_PROCS     → macOS-имена процессов (.app без .exe)

Разрешения macOS (TCC — Transparency, Consent & Control):
  Системные настройки → Конфиденциальность → Мониторинг ввода,
  Полный доступ к диску, Специальные возможности.
  Без этих разрешений CGWindowListCopyWindowInfo вернёт пустой список,
  а чтение заголовков чужих окон будет заблокировано.
"""
...
# Системные пути macOS — изменения в них игнорируются (шум)
SKIP_PREFIXES = (
    "/System",
    "/Library",
    "/private/var",
...
# Подозрительные процессы для macOS (имена без .exe — в macOS это bundle-имена)
SUSPICIOUS_PROCS = {
    # Архиваторы
    "Archive Utility", "The Unarchiver", "Keka", "Archiver 4", "BetterZip 5",
    # Мессенджеры
    "Telegram", "Discord", "WhatsApp",
    # Облачные хранилища
    "Dropbox", "OneDrive", "Google Drive",
    # Передача файлов
    "Cyberduck", "FileZilla", "Transmit 5", "ForkLift", "Mountain Duck",
    # Терминалы (высокий риск: scp, curl, rsync без GUI)
    "Terminal", "iTerm2", "Warp", "Hyper",
    # SSH/SFTP CLI-процессы
...
# ─────────────────────────────────────────────────────────────────────────────
# Модуль 1: База данных (thread-safe SQLite) — идентична Windows-версии
# ─────────────────────────────────────────────────────────────────────────────
...
class Database:
    ...
    def log_process_event(self, event_type, name, pid,
                          username="", cmdline="", ppid=None, risk=0.0):
        with self._cursor() as cur:
            cur.execute(
                "INSERT INTO process_events "
                "(timestamp,event_type,process_name,process_id,username,command_line,parent_pid,risk_score)"
                " VALUES (?,?,?,?,?,?,?,?)",
                (datetime.now().isoformat(), event_type, name, pid,
                 username, cmdline, ppid, risk),
            )

    def log_file_event(self, event_type, file_path,
                       file_size=0, is_directory=False, risk=0.0):
        with self._cursor() as cur:
            cur.execute(
    ...
    def log_usb_event(self, event_type, device_id,
                      device_name="", vendor_id="", product_id="", risk=0.0):
        with self._cursor() as cur:
            cur.execute(
    ...
    def log_user_activity(self, window_title, process_name, process_id, duration_seconds=0):
        with self._cursor() as cur:
            cur.execute(
    ...
    def get_high_risk_events(self, threshold=0.5, limit=50):
        with self._cursor() as cur:
            cur.execute(f"""
                SELECT timestamp, 'process' AS source, event_type,
                       process_name AS detail, risk_score
...
def calc_file_risk(event_type: str, file_path: str) -> float:
    ...
    # На macOS съёмные диски монтируются в /Volumes/
    if file_path.startswith("/Volumes/"):
        score += 0.4   # запись на внешний диск
    if _off_hours():
...
# ─────────────────────────────────────────────────────────────────────────────
# Модуль 3: Мониторинг активных окон (macOS)
# ─────────────────────────────────────────────────────────────────────────────

class UserActivityMonitor:
    """
    macOS-аналог Windows GetForegroundWindow().

    Используемые API:
    ┌─────────────────────────────────────────────────────────────────┐
    │ AppKit.NSWorkspace.sharedWorkspace().frontmostApplication()     │
    │   → NSRunningApplication с полями localizedName, processID     │
    │                                                                 │
    │ Quartz.CGWindowListCopyWindowInfo(                              │
    │     kCGWindowListOptionOnScreenOnly, kCGNullWindowID           │
    │   )                                                             │
    │   → CFArray словарей со свойствами всех видимых окон           │
    │     kCGWindowName       — заголовок окна (строка)              │
    │     kCGWindowOwnerPID   — PID процесса-владельца               │
    │     kCGWindowLayer      — Z-порядок (0 = нормальное окно)      │
    └─────────────────────────────────────────────────────────────────┘

    Требует разрешения "Screen Recording" в TCC (System Preferences →
    Privacy → Screen Recording). Без него CGWindowListCopyWindowInfo
    возвращает только имена приложений, но не заголовки окон.

    Альтернатива для случая без TCC:
        NSWorkspace.sharedWorkspace().activeApplication()
    возвращает словарь {NSApplicationName, NSApplicationProcessIdentifier, ...}
    без заголовка окна — менее информативно, но без спецразрешений.
    """
    ...
    def __init__(self, db: Database, poll_interval: float = 2.0):
        ...
        self.running = False
        self._state = {"pid": None, "title": "", "proc": "", "since": None}
    ...
    def _active_window(self) -> dict | None:
        try:
            ...
            # 1. Определяем frontmost-приложение
            ...
            proc_name = app.localizedName() or "unknown"

            # 2. Ищем заголовок главного окна среди всех видимых окон
            #    CGWindowListCopyWindowInfo обходит список в Z-порядке (front→back)
            ...
            for w in windows:
                if w.get("kCGWindowOwnerPID") == pid:
                    # kCGWindowLayer == 0 — обычные окна приложения
                    if w.get("kCGWindowLayer", 1) == 0:
                        title = w.get("kCGWindowName") or ""
                        if title:
                            break  # берём первое окно (frontmost)

            if not title:
                title = proc_name   # фолбэк: имя приложения

            return {"pid": pid, "title": title, "proc": proc_name}

        except ImportError:
            # pyobjc не установлен — тихий фолбэк через AppleScript
            return self._active_window_applescript()
        except Exception:
            return None

    def _active_window_applescript(self) -> dict | None:
        """
        Резервный метод: AppleScript не требует pyobjc.
        Медленнее (~50 мс vs ~1 мс для Quartz), но надёжен.
        """
        try:
            script = '''
    ...
    def _loop(self):
        while self.running:
            try:
                ...
                if info and info["pid"] != self._state["pid"]:
                    # Сохраняем длительность предыдущего окна
                    if self._state["pid"] is not None and self._state["since"]:
                        duration = int(
    ...
    def start(self):
        ...
        print("[+] UserActivityMonitor запущен (AppKit/Quartz)")
...
# ─────────────────────────────────────────────────────────────────────────────
# Модуль 4: Мониторинг процессов — psutil, кросс-платформенный
# ─────────────────────────────────────────────────────────────────────────────

class ProcessMonitor:
    """
    Snapshot-diff подход: сравниваем множества PID на каждом тике.
    psutil на macOS использует:
        sysctl(CTL_KERN, KERN_PROC, KERN_PROC_ALL) → список kinfo_proc
    Это аналог CreateToolhelp32Snapshot на Windows.

    Альтернатива с событиями (без polling):
        kqueue + EVFILT_PROC + NOTE_FORK/NOTE_EXIT — нативный механизм
        BSD-уведомлений о процессах. Требует отдельного event loop.
        psutil проще и достаточен для учебного прототипа.
    """
        ...
        while self.running:
            try:
                ...
                for pid in old_pids - new_pids:
                    ...
                    if info:
                        self.db.log_process_event(
                            "terminated", info["name"], info["pid"], risk=0.0
                        )
    ...
    def start(self):
        ...
        print("[+] ProcessMonitor запущен (psutil / sysctl)")
...
# ─────────────────────────────────────────────────────────────────────────────
# Модуль 5: Мониторинг файловой системы — watchdog (FSEvents backend)
# ─────────────────────────────────────────────────────────────────────────────

class FileSystemMonitor:
    """
    watchdog на macOS автоматически использует FSEvents backend
    (вместо ReadDirectoryChangesW на Windows).

    FSEvents — kernel-level API Apple:
        FSEventStreamCreate() регистрирует callback на потоке run loop.
        Ядро батчует уведомления (batch latency) и доставляет их пачками,
        что эффективнее поштучной доставки WinAPI-аналогов.

    Важная разница: FSEvents не даёт точный тип изменения для каждого файла
    по умолчанию — watchdog внутри доополнительно stat()-ирует пути.
    С флагом kFSEventStreamCreateFlagFileEvents (watchdog включает его)
    события приходят на уровне файлов, не директорий.

    Пути мониторинга по умолчанию:
        ~/Desktop, ~/Documents, ~/Downloads, /Volumes (USB-диски)
    """
    ...
    def __init__(self, db: Database, watch_paths: list[str] = None):
        ...
        if watch_paths is None:
            ...
            watch_paths = [
                ...
                "/Volumes",   # все смонтированные внешние диски
            ]
    ...
    def start(self):
        try:
            ...
            for path in self.watch_paths:
                ...
                print(f"[+] FileSystemMonitor (FSEvents): {path}")
            self.observer.start()
        except ImportError:
...
# ─────────────────────────────────────────────────────────────────────────────
# Модуль 6: Мониторинг USB (macOS)
# ─────────────────────────────────────────────────────────────────────────────

class USBMonitor:
    """
    macOS-замена WMI Win32_PnPEntity.

    Реализован как snapshot-diff поверх `system_profiler SPUSBDataType -json`.
    system_profiler — Apple CLI-утилита, читающая IOKit Registry напрямую.

    ═══ Техническое сравнение подходов ══════════════════════════════════════
    Подход 1 (использован здесь): polling system_profiler
      + нет зависимостей (только subprocess)
      + стабильно на всех версиях macOS
      - опрос раз в 2 с (задержка обнаружения)
      - fork+exec на каждый тик (≈30 мс CPU)

    Подход 2: IOKit notifications через pyobjc (production-уровень)
      Использует IOServiceAddMatchingNotification + CFRunLoop:
        matching = IOServiceMatching("IOUSBHostDevice")
        IOServiceAddMatchingNotification(
            notify_port, kIOFirstMatchNotification,
            matching, callback, None, iterator
        )
      + событийная модель, нет polling
      + нулевая нагрузка в ожидании
      - требует pyobjc + runloop в отдельном потоке
      - сложнее в обработке ошибок

    Подход 3: launchd + ioreg + FSEvents на /dev
      Следит за появлением /dev/diskN — косвенный признак подключения.
      Не даёт VID/PID без ioreg-запроса.
    ═════════════════════════════════════════════════════════════════════════

    Формат system_profiler JSON (упрощённо):
    {
      "SPUSBDataType": [
        {
          "_name": "USB 3.1 Bus",
    ...
    POLL_INTERVAL = 2.0   # секунды между проверками

    def __init__(self, db: Database):
        self.db = db
        self.running = False
        self._snapshot: dict[str, dict] = {}   # device_id → device_info

    # ── Парсинг дерева USB из system_profiler ─────────────────────────────────

    @staticmethod
    def _flatten(items: list, result: dict):
        """Рекурсивно обходит вложенное дерево USB-хабов и устройств."""
        for item in items or []:
            ...
            # Уникальный ключ устройства (location_id — слот на шине)
            device_id = f"{vid}:{pid}:{loc_id}" if loc_id else f"{vid}:{pid}:{name}"

            # Реальные устройства имеют vendor_id или product_id
            if vid or pid:
                result[device_id] = {
                    "name": name,
                    "vendor_id": vid,
            ...
            # Рекурсия в дочерние хабы/_items
            USBMonitor._flatten(item.get("_items", []), result)

    def _get_usb_devices(self) -> dict[str, dict]:
        """Возвращает плоский словарь {device_id: info} текущих USB-устройств."""
        try:
            result = subprocess.run(
            ...
            self._flatten(data.get("SPUSBDataType", []), devices)
            return devices
        except Exception:
            return {}

    def _loop(self):
        print("[+] USBMonitor запущен (system_profiler polling)")
        self._snapshot = self._get_usb_devices()

        while self.running:
            time.sleep(self.POLL_INTERVAL)
            try:
                current = self._get_usb_devices()
                prev_ids = set(self._snapshot)
                curr_ids = set(current)

                # Новые устройства
                for dev_id in curr_ids - prev_ids:
                    info = current[dev_id]
                    risk = calc_usb_risk("connected")
                    self.db.log_usb_event(
                        "connected", dev_id,
                        info["name"], info["vendor_id"], info["product_id"], risk
                    )
                    print(f"[USB] Подключено: {info['name']} "
                          f"(VID={info['vendor_id']} PID={info['product_id']})")

                # Удалённые устройства
                for dev_id in prev_ids - curr_ids:
                    info = self._snapshot[dev_id]
                    risk = calc_usb_risk("disconnected")
                    self.db.log_usb_event(
                        "disconnected", dev_id,
                        info["name"], info["vendor_id"], info["product_id"], risk
                    )
                    print(f"[USB] Отключено: {info['name']}")
...
# ─────────────────────────────────────────────────────────────────────────────
# Модуль 7: Веб-дашборд (Flask) — идентичен Windows-версии
# ─────────────────────────────────────────────────────────────────────────────
...
<style>
  ...
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .card-label { font-size: 10px; color: var(--muted); text-transform: uppercase; letter-spacing: 1px; margin-bottom: 8px; }
  .card-value { font-size: 34px; font-weight: 700; color: var(--heading); font-family: 'Syne', sans-serif; line-height: 1; }
  ...
  .risk-fill  { height: 6px; border-radius: 3px; min-width: 4px; }
  ...
  .hm-cell { height: 28px; border-radius: 4px; background: var(--border); position: relative; cursor: default; transition: transform .15s; }
  .hm-cell:hover { transform: scaleY(1.15); }
  .hm-cell .hm-tip { display: none; position: absolute; bottom: calc(100% + 6px); left: 50%; transform: translateX(-50%); background: #1e2230; border: 1px solid var(--border); border-radius: 6px; padding: 4px 8px; font-size: 10px; white-space: nowrap; pointer-events: none; z-index: 10; }
  .hm-cell:hover .hm-tip { display: block; }
  .hm-hours { display: grid; grid-template-columns: repeat(24, 1fr); gap: 3px; margin-top: 4px; }
  .hm-h { text-align: center; font-size: 9px; color: var(--muted); }

  .proc-bar-row { display: flex; align-items: center; gap: 10px; margin-bottom: 8px; }
  .proc-name  { width: 150px; flex-shrink: 0; overflow: hidden; text-overflow: ellipsis; font-size: 12px; color: var(--text); }
  .proc-track { flex: 1; height: 10px; background: var(--border); border-radius: 5px; overflow: hidden; }
  .proc-fill  { height: 100%; border-radius: 5px; background: linear-gradient(90deg, var(--accent), #0077ff); transition: width .5s ease; }
  .proc-cnt   { width: 36px; text-align: right; font-size: 11px; color: var(--muted); }
...
<header>
  ...
  <div>
    <h1>DLP Monitor</h1>
    <div class="sub">Data Loss Prevention · macOS · Prototype</div>
  </div>
...
<div class="main" id="grid">
  ...
  <div class="card col-wide">
    ...
    <div class="scrollable">
      <table class="event-table">
        <thead><tr><th>Время</th><th>Тип</th><th>Событие</th><th>Детали</th><th>Риск</th></tr></thead>
  ...
  <div class="card col-full">
    ...
    <div class="scrollable">
      <table class="event-table">
        <thead><tr><th>Время</th><th>Событие</th><th>Устройство</th><th>Device ID</th><th>VID</th><th>PID</th><th>Риск</th></tr></thead>
...
const badgeSource = { process:'badge-process', file:'badge-file', usb:'badge-usb', activity:'badge-activity' };
const badgeEvent  = (t) => {
...
const riskColor = (r) => r >= 0.7 ? '#ff3b5c' : r >= 0.4 ? '#ffb830' : '#00e676';
const riskBar   = (r) => {
  const pct = Math.round(r * 100);
  return `<div class="risk-bar"><div class="risk-fill" style="width:${pct}px;background:${riskColor(r)}"></div><span class="risk-label">${pct}%</span></div>`;
};
const makeRow = (e) => `<tr>
...
function buildHeatmap(data) {
  ...
  for (let h=0; h<24; h++) {
    ...
    cell.style.opacity = 0.1 + pct * 0.85;
...
function buildTopProcs(data) {
  ...
  document.getElementById('top-procs').innerHTML =
    data.map(p => `<div class="proc-bar-row">
      ...
      <div class="proc-track"><div class="proc-fill" style="width:${Math.round(p.cnt/max*100)}%"></div></div>
...
def parse_args():
    p = argparse.ArgumentParser(
        description="DLP-агент + веб-дашборд (macOS)",
...
def main():
    ...
    print("=" * 62)
    print("  DLP-монитор — macOS-порт прототипа агента сбора данных")
    print(f"  БД: {os.path.abspath(args.db)}")
    print(f"  Платформа: {platform.system()} {platform.release()} {platform.machine()}")
    print("=" * 62)

    if not IS_MACOS:
        print("[WARN] Система не macOS. Часть модулей может не работать.")

    print()
    print("  Необходимые разрешения macOS (TCC):")
    print("  • Screen Recording  — для заголовков окон (CGWindowListCopyWindowInfo)")
    print("  • Full Disk Access  — для мониторинга файлов в ~/Documents, ~/Desktop")
    print("  • Accessibility     — для AppleScript-фолбэка (osascript)")
    print()
